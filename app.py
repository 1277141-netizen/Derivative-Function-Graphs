...
with col1:
    ...
    derivative_input = st.text_input(
        "Enter the derivative expression (use x, e.g., 2*x):",
        value="2*x"
    )
...
def parse_conditions(text, f_expr, fp_expr):
    ...
    for line in text.splitlines():
        ...
        try:
            right_val = sp.sympify(right.strip())
        except:
            st.warning(f"Cannot parse RHS value: {right}")
            continue

        if "f'(" in left:
            try:
                val = sp.sympify(left[left.find("(")+1:left.find(")")])
                equations.append(fp_expr.subs(x, val) - right_val)
            except:
                st.warning(f"Cannot parse initial condition: {line}")
        elif "f(" in left:
            try:
                val = sp.sympify(left[left.find("(")+1:left.find(")")])
                equations.append(f_expr.subs(x, val) - right_val)
            except:
                st.warning(f"Cannot parse initial condition: {line}")
    return equations
...
try:
    ...
    if zeros_text.strip():
        for z in zeros_text.split(","):
            try:
                z_val = sp.sympify(z.strip())
                equations.append(f_prime.subs(x, z_val))
            except:
                st.warning(f"Cannot parse zero value: {z}")
    ...
    # Critical & inflection points safely
    critical_points = []
    for c in sp.solve(f_prime, x):
        try:
            c_val = float(c.evalf())
            critical_points.append(c_val)
        except:
            pass

    inflection_points = []
    for i in sp.solve(f_double_prime, x):
        try:
            i_val = float(i.evalf())
            inflection_points.append(i_val)
        except:
            pass
    ...
    axes[1].plot(xs, yps, label="f'(x)")
    axes[1].set_ylabel("f'(x)")
    axes[1].grid(True)

    axes[2].plot(xs, ypps, label="f''(x)")
    ...
    for c in critical_points:
        if x_min <= c <= x_max:
            axes[0].plot([c, c], [f_func(c), 0], linestyle="--", color='red')
            axes[1].plot([c, c], [0, 0], linestyle="--", color='red')

    for i in inflection_points:
        if x_min <= i <= x_max:
            axes[0].plot([i, i], [f_func(i), 0], linestyle="--", color='green')
            axes[2].plot([i, i], [0, 0], linestyle="--", color='green')
